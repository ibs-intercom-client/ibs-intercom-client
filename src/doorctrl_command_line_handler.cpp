...
doorctrl::DoorctrlCommandLineOptions doorctrl::DoorctrlCommandLineHandler::handleArgs(int argc, char** argv) {

   doorctrl::DoorctrlCommandLineOptions options;
   ...
   for(int i = 1; i < argc; i++) { 
      ...
      } else if(strncmp(argv[i], "-k", 3) == 0 || strncmp(argv[i], "--kiosk", 8) == 0) { 
         options.kiosk_mode = true; 
...
void doorctrl::DoorctrlCommandLineHandler::printVersionText() {
  std::cout << doorctrl::DOORCTRL_APPNAME << " " << doorctrl::DOORCTRL_VERSION << std::endl;
}


void doorctrl::DoorctrlCommandLineHandler::printHelpText() {
  std::cout << doorctrl::DOORCTRL_APPNAME << " " << doorctrl::DOORCTRL_VERSION << std::endl;
  std::cout << "Usage: " << doorctrl::DOORCTRL_BINNAME << " [OPTIONS]..." << std::endl;
  std::cout << "   -d, --console-debug=LEVEL  set console debug level:" << std::endl; 
  std::cout << "                              0 = none, 1 = critical, ..., 5 = debug" << std::endl;
  std::cout << "   -k, --kiosk                run in kiosk mode: open main window in" << std::endl; 
  std::cout << "                              fullscreen mode and hide window controls" << std::endl;
  std::cout << "   -v, --version              print doorctrl version" << std::endl;
